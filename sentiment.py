...
def get_all_articles(topic):
    ...
    all_articles = newsapi.get_everything(qintitle=topic, from_param=week_ago_date, to=
                                          today_date, language='en', sort_by='relevancy')
...
def get_articles_sentiment(topic):
    ...
    sentiments = {'positive': [], 'neutral': [], 'negative': [],
                  'overallPositive': 0, 'overallNeutral': 0, 'overallNegative': 0}
    ...
    sentiments['overallPositive'] = "{:.5f}".format(sum(sentiments['positive']) / resultsLength)
    sentiments['overallNegative'] = "{:.5f}".format(sum(sentiments['negative']) / resultsLength)
    sentiments['overallNeutral'] = "{:.5f}".format(sum(sentiments['neutral']) / resultsLength)
